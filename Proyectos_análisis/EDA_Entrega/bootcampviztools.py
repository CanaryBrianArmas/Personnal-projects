...
def show_distribution_categorical(df, columnas_categoricas, relativa = False, mostrar_valores = False):
    """
    Dibuja una distribución de categorías para cada columna de un dataframe.

    Parameters:

    df : pandas.DataFrame
        Dataframe que contiene las columnas cuyas distribuciones se quieren dibujar.
    columnas_categoricas : list
        Lista de nombres de las columnas cuyas distribuciones se quieren dibujar.
    relativa : bool, optional
        Si es True, se dibujan las frecuencias relativas en lugar de las frecuencias absolutas.
        Por defecto es False.
    mostrar_valores : bool, optional
        Si es True, se muestran los valores de las frecuencias en el gráfico.
        Por defecto es False.
...
def plot_categorical_relationship_fin(df, cat_col1, cat_col2, relative_freq = False, show_values = False, size_group = 5):
    # Prepara los datos
    """
    Dibuja una gráfica de barras para mostrar la relación entre dos columnas categóricas de un dataframe.
    ...
    df : pandas.DataFrame
        Dataframe que contiene las columnas cuyas distribuciones se quieren dibujar.
    cat_col1 : str
        Nombre de la columna categórica que se quiere dibujar en el eje X.
    cat_col2 : str
        Nombre de la columna categórica que se quiere dibujar en el eje Y.
    ...
    size_group : int, optional
        Número de categorías que se quieren mostrar en cada gráfico.
        Si el número de categorías en cat_col1 es mayor que size_group, se crean varios gráficos.
        Por defecto es 5.
...
def plot_categorical_numerical_relationship(df, categorical_col, numerical_col, show_values = False, measure = 'mean'):
    ...
    """
    Plots the relationship between a categorical column and a numerical column
    using bar plots. The plot displays either the mean or median of the numerical
    ...
    show_values (bool, optional): If True, displays the values on top of the bars.
                                  Defaults to False.
    measure (str, optional): Statistical measure to display ('mean' or 'median').
                             Defaults to 'mean'.

    The function divides categories into groups of 5 if there are more than 5 categories,
    creating a separate plot for each group. Each plot displays the measure of central
    tendency for the numerical column for the categories in that group.
    """
    if measure == 'median':
...
def plot_combined_graphs(df, columns, whisker_width = 1.5, bins = None):
    """
    Crea un gráfico con dos subplots por columna en df. 
    El primer subplot es un histograma y una curva de densidad de 
    la columna dada. El segundo subplot es un boxplot de la misma columna.
    
    Parameters:
    df (pandas.DataFrame): Dataframe que contiene las columnas a analizar.
    columns (list): Lista de nombres de columnas a analizar.
    whisker_width (float, optional): Ancho de los bigotes en el boxplot. 
        Por defecto es 1.5.
    bins (int, optional): Número de bins en el histograma. Si no se proporciona, 
        se usan los bins predeterminados de seaborn.
    """
...
def plot_grouped_boxplots(df, cat_col, num_col):
    """
    Crea un gráfico con boxplots para cada categoría en una columna categorizadora.
    El gráfico se divide en subgrupos de 5 categorías cada uno, para que no se
    saturen los gráficos con demasiadas categorías.
    
    Parameters:
    df (pandas.DataFrame): Dataframe que contiene las columnas a analizar.
    cat_col (str): Nombre de la columna categorizadora.
    num_col (str): Nombre de la columna numérica cuyos boxplots se quieren dibujar.
    """
...
def plot_grouped_histograms(df, cat_col, num_col, group_size):
    """
    Crea un gráfico con histogramas para cada categoría en una columna categorizadora.
    El gráfico se divide en subgrupos de group_size categorías cada uno, para que no se
    saturen los gráficos con demasiadas categorías.
    
    Parameters:
    df (pandas.DataFrame): Dataframe que contiene las columnas a analizar.
    cat_col (str): Nombre de la columna categorizadora.
    num_col (str): Nombre de la columna numérica cuyos histogramas se quieren dibujar.
    group_size (int): Número de categorías por grupo.
    """
    unique_cats = df[cat_col].unique()
    num_cats = len(unique_cats)
